...
@Component({
    ...
    <section class="landing-card hero-section" id="home">
      <div class="hero-copy">
        ...
        <form class="market-search" appScrollReveal="fade-up" [revealDelay]="340">
          ...
          <a routerLink="/catalogue">Search marketplace</a>
        </form>
      ...
      <div class="hero-collage" [class.collage-ready]="collageReady">
        ...
        @for (item of collage; track item.alt; let index = $index) {
          <figure class="collage-card" [class]="'card-' + (index + 1)">
  ...
  .eyebrow{color: #ff9700;}
    ...
    .collage-card { background: #ecebe8; border-radius: 20px; box-shadow: 0 28px 55px rgba(0,0,0,.16); height: clamp(145px, 20vw, 220px); left: 50%; margin: 0; overflow: hidden; position: absolute; top: 52%; transform-origin: center bottom; width: clamp(160px, 22vw, 245px); }
    ...
    @media (max-width: 560px) {
      ...
      .market-search { grid-template-columns: auto 1fr; padding: .7rem 1rem; }
      .market-search a { grid-column: 1 / -1; text-align: center; }
